...
#define ID_MAX 6 /* Guarda tamanho dos nomes dos IDs */
...
int check_all_aeroporto (char ID[ID_MAX]) {
  ...
  /* Verifica se o ID estah em uppercase e lowercase */
  for (i = 0; i < size; i++) {
    if ((ID[i] < 'a' || ID[i] > 'z') &&
    (ID[i] < 'A' || ID[i] > 'Z')) {
      printf(ERROR_A_ID);
...
void lista_voo_partida() {
  ...
  for (i = 0; i < h; i++){
    k = vector_ordem[i];
    if (Voos[k].Data[0] == Data[0] && Voos[k].Data[1] == Data[1] && 
    Voos[k].Data[2] == Data[2]) {
...
void elimina_reserva() {
  ...
  if (strlen(Codigo) <= 6) {
    for (i = 0; i < Voos_Ocupados; i++) {
      if ((strcmp(Voos[i].Codigo_Voo, Codigo) == 0) && 
      ((Voos[i].Data[2] < Data [2]) || 
      (Voos[i].Data[2] == Data [2] && Voos[i].Data[1] < Data [1]) ||
      (Voos[i].Data[2] == Data [2] && Voos[i].Data[1] == Data [1] &&
      Voos[i].Data[0] < Data [0]))) {
        free_reservas(i);
